...
module.exports = function(Square) {
		...
		if (remaining < 0) {
			seq.forEach((d, i) => {
				if (d.pressure < remaining) {
					if (that.log) {
						console.log("Updating pressure at " + id + "-" + i + " from " + d.pressure + " to " + remaining);
					}
	...
	Square.prototype.setTile = function(c, i) {
		var p = this.square[c][i].pressure;
		if (p > 0) {
